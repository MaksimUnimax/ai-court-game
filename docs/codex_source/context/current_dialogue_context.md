...
## CTX-2026-05-23-001 — Сохранение и удаление активного сценария

Источник: замечание пользователя после проверки загрузки пакета дела.

Пользователь указал, что при обновлении страницы активный сценарий слетает.

Нужно сделать так, чтобы загруженный сценарий сохранялся после refresh.

Также нужна возможность удалить активный сценарий и загрузить новый.

Сохранение должно быть локальным в браузере, без серверного хранения файлов.

Нужно сохранять сценарий, изображения, статус пакета и желательно текущее состояние прохождения.

Из-за размера изображений предпочтительно использовать IndexedDB, а не localStorage.

В UI должна быть понятная кнопка удаления активного дела/сценария.
